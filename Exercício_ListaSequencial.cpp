    ...
    while( 1 ){
         ...
         switch( op ) {
            case 1:  // rotina cria lista, inicializa a lista
                    cria_lista( &lista.f );//chama a função
                    break;
 ...
 ************************************************/
void cria_lista( int *f){

    *f = 0;                                   // iniciliza lista, tamanho da lista = 0
    printf( "\n Lista criada! " );
}
...
void exclui_nodo( NODO *lista ){
    if(lista->f ==0)
        ...
        if((pf >=0 ) && (pf < lista->f) && ( lista->info[pf].codigo==cod)){
            for(i= pf; i < lista->f -1; i++)
                lista->info[i]=lista->info[i + 1];
                lista->f--;
                printf("Resgistro excluido");
            }else
                  printf("Nodo de referencia nao existe!");
    }
}
...
void ordena_lista( NODO *lista ){
    ...
    int i, j, aux;                  // Algoritmo Método Bolha - Bubble Sort

     for(i=0;i<lista->f;i++){   //faz referencia as posições da lista, no caso i = posição[0], se houver validcao passa para proxima prosição e compara dns
     	for(j=0;j<lista->f-i-1;j++){ //esse for faz comparação com i do for de cima até a penultima posicao, por que o j+1 da linha 302 que é a ultima posicao faz comparação com antipenultima e por assim a ultima posicao
     		if(lista->info[ j ].codigo>lista->info[ j+1 ].codigo){ // if fazemos a comparação  de j com j+1 s ele for maior fazemos a troca
     			aux = lista->info[ j ].codigo; // varialvel aux recebe o que ta em j
     			lista->info[ j ].codigo = lista->info[ j+1 ].codigo;// j recebe o que ta em j+1, ou seja a posição seguinte vem para ele
     			lista->info[ j+1 ].codigo = aux; // e j+1 recebe aux,  a posicao anterior vai para proxima, realizando a troca
			 }
		 }
	 }

// da linha 300 até a linha 302 é a troca  da linha 297 e 298 é os for para percorrer a lista e comparar os i e o j e na linha 299 temos a verificaçao!!
}
//int aux, i, j; // algoritimo principal, foi a primeira logica usado a principio, porem o algoritimo a cima foi o implementado no codigo!!

	//for(j=0;j<lista->f-i-1;j++){
		//for(i=0; i<j; i++){
            //if(lista->f[i]>lista->f[i+1]){
                //aux=lista->f[i];
                //lista->f[i]=lista->f[i+1];
                //lista->f[i+1]=aux;
...
if( lista->f ==0)
        printf("\n Lista vazia!");
            ...
            if( ( pf>= 0) && (pf < lista ->f ) && (lista-> info[pf].codigo == cod)){
                  printf("\n Informe campo que será alterado:                      ");
                  printf("\n [1] Nome                                              ");
                  printf("\n  /---------------------------------------------------/");
                  printf("\n Opções: ");
                  fflush(stdin);
           ...
           printf("\n Registro de referencia não encontrado!");
    }
}
...
void consulta_nome( NODO lista ){
    ...
    if(i >= lista.f)
        ...
        printf("Nome: %s - Código; %d", lista.info[i].nome, lista.info[i].codigo);
}
